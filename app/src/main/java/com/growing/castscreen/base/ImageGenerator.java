...
import com.growing.castscreen.services.CastScreenServerService;
...
//                Log.e(TAG, "ImageGenerator.onImageAvailable Tread: " + Thread.currentThread().getName());
                ...
                resizedBitmap.compress(Bitmap.CompressFormat.JPEG, getPreferencesHelper().getJpegQuality(), mJpegOutputStream);
    ...
    public void start() throws IllegalStateException {
        synchronized (mLock) {
            ...
            final MediaProjection mediaProjection = CastScreenServerService.getMediaProjection();
            if (mediaProjection == null) throw new IllegalStateException("MediaProjection is null");

            final Matrix matrix = new Matrix();
            if (getPreferencesHelper().getResizeFactor() != PreferencesHelper.DEFAULT_RESIZE_FACTOR) {
                final float scale = getPreferencesHelper().getResizeFactor() / 10f;
